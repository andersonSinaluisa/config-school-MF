...
import { EditLevelCommand } from "@/scolar/application/useCases/levels/editLevelUseCase"
import { AlertCircle, Check, ChevronRight, HelpCircle, Home, Info, Save, ShieldCheck } from "lucide-react"
import { FieldErrors, UseFormRegister } from "react-hook-form"
import { Link } from "react-router-dom"

interface EditLevelPresenterProps {
    formData: EditLevelCommand
    isSubmitting: boolean
    onSubmit: (e: React.FormEvent<HTMLFormElement>) => void
    onCancel: () => void
    register: UseFormRegister<EditLevelCommand>
    errors: FieldErrors<EditLevelCommand>
}
export const EditLevelPresenter = ({
    formData,
    isSubmitting,
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        id="order"
                                        type="number"
                                        {...register("data.order", { required: true, valueAsNumber: true })}
                                        min="1"
                                    />
                                    {
                                        errors.data?.order && (
                                            <span className="text-red-500 text-sm">
                                                {errors.data.order.message}
                                            </span>
                                        )
                                    }
                                </div>
